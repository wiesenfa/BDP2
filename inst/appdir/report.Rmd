---
title: "BDP2 workflow"
author: "created by BDP2 package (Annette Kopp-Schneider, Manuel Wiesenfarth)"
...
- Uninteresting and target response rate: p0=`r p0`, p1=`r p1`
...
- pF=`r pF` and cF=`r cF`, i.e. futility criterion $P(p>$ `r pF` $|Data) <$ `r cF`
- pE=`r pE` and cE=`r cE`, i.e. efficacy criterion $P(p>$ `r pE` $|Data) \geq$ `r cE`
...
par(mfrow=c(1,2))
plotBDP2(x="n",y="Prob0Successes",n=c(n.range_1[1],n.range_1[2]),p0=p0,p1=p1,cex.main=.9)
plotBDP2(x="n",y="PostProb0or1Successes",n=c(n.range_1[1],n.range_1[2]),pF=pF,shape1F=shape1F,shape2F=shape2F,cex.main=.9)
...
Threshold $c_E$ selected such that type I error probability can be tolerated and power is maximized.

The selected cE leads to a type I error of `r round(res$y.p0[which.min(abs(res$x.p0-cE))[1]],2)` at p0=`r p0`,
and a power of `r round(res$y.p1[which.min(abs(res$x.p1-cE))[1]],2)` at p1=`r p1`.
...
plotBDP2(x="n",y="bFbE",n=n,pF=pF,cF=cF,pE=pE,cE=cE,
          shape1F=shape1F,shape2F=shape2F,shape1E=shape1E,shape2E=shape2E,col=c("red","green"),cex.sub=.8)
