plotPEstop_p0_p1VSn=function(nvec,vn.int,pF,cF,pE,cE,p0,p1,shape1F,shape2F,shape1E=NULL,shape2E=NULL,col1="green",col2="red",cex.legend=1,show=TRUE,...){
  ...
  out=lapply(nvec, function(n) {
    ...
    res=c(pFstopEstop(p0,c(vn.int1,n),v.critE,v.critF)$P.effic.cum[length(vn.int1)+1],
          pFstopEstop(p1,c(vn.int1,n),v.critE,v.critF)$P.effic.cum[length(vn.int1)+1])
  ...
  legend("topleft",legend=c("Cumulative Efficacy at Final for ptrue=p0","Cumulative Efficacy at Final for ptrue=p1" ),lty=1,col = c(col2,col1),cex=par()$cex*cex.legend)

  class(res)="n_vs_pEstop_p0_p1"
  invisible(res)
}
