pFstopEcall_vec=function(n, p, pE,pF, vn.int, cEvec, cF, shape1F,shape2F,shape1E=NULL,shape2E=NULL,printit=FALSE){
  ...
  out=lapply(cEvec, function(cE) {
    ...
    res <- pFstopEcall(p,c(vn.int,n),v.critE,v.critF)
    res
  }
  )
  names(out)=cEvec
...
#  summ=sapply(out, function(x) x$P.stop.cum[c(1,length(vn.int)+1)])
  summ=sapply(out, function(x) x$P.effic)
